...
# applying filter to incoming stream
...
async def clickevent(clickevents):
    ...
    #
    async for clickevt in clickevents.filter(lambda x:x.number >= 100):
    #
        # TODO: Send the message to the `popular_uris_topic` with a key and value.
        #
        await popular_uris_topic.send(key=clickevt.uri, value=clickevt.number)
        print(json.dumps(asdict(clickevt), indent=2))
